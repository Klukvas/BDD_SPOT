...
def enc_data(get_enc_key):
    enc_data = Circle().encrypt_data(get_enc_key[0], get_enc_key[1]['encryptionKey'])['response']
...
def add_bank_account(auth, bank_country, billing_country, account_number, routing_number, iban, guid):
    ...
    response = Circle().add_bank_account(token, bank_country, billing_country,
                                         account_number, iban, routing_number, guid)['response']
    return response
...
def delete_card(get_enc_key, add_card):
    deleted_card = Circle().delete_card(get_enc_key[0], add_card['id'])['response']
    assert deleted_card['data']['deleted'] == True, f"Expected that "
    all_cards = Circle().get_all_cards(get_enc_key[0])['response']
    assert len(
        list(
            filter(
...
def add_bank_account(auth, variables_dict):
    ...
    response = Circle().add_bank_account(token, variables_dict['bank_country'], variables_dict['billing_country'],
                                         ...
                                         variables_dict['routing_number'], variables_dict['guid'])['response']
    return response
...
def check_response_status_code(status_code, response):
    assert int(status_code) == int(response['status']), f'status code is different from expected. Expected {status_code}'
...
def check_add_bank_account_response_data(variables_dict, response):
    ...
    assert isinstance(response_data['bankAddressDistrict'], str), f'add_bank_account response is not ok. BankAddressDistrict is {response_data["bankAddressDistrict"]}, expected string'
...
def get_all_bank_accounts(auth, count):
    ...
    response = Circle().get_bank_account_all(token)['response']
    response['token'] = token
    response['accounts_count'] = len(response['data'])
    try:
        response['bankaccountid_to_delete'] = response['data'][-1]['bankAccountId']
    except IndexError:
        pass
    return response
...
def check_all_bank_accounts(response):
    assert response['result'] == 'OK', f'get-bank-account-all response is not ok. Result is {response["result"]}, expected OK'
    response_data = response['data']
...
def get_bank_account(auth, state):
    if state != 'with no auth token':
        ...
        if state == 'if it exist':
            response = Circle().get_bank_account(token, settings.circle_my_bank_account_id)['response']
        elif state == "if bank_account_id is not my":
            response = Circle().get_bank_account(token, settings.circle_not_my_bank_account_id)['response']
        elif state == 'if bank_account_id was deleted':
            response = Circle().get_bank_account(token, settings.circle_deleted_bank_account_id)['response']
        elif state == 'if bank_account_id is invalid':
            response = Circle().get_bank_account(token, settings.circle_invalid_bank_account_id)['response']
    elif state == 'with no auth token':
        response = Circle().get_bank_account('', settings.circle_my_bank_account_id, True)

    return response

@then('response must contains info about bank account')
def check_get_bank_account(response):
    assert response['result'] == 'OK', f'get-bank-account response is not ok. Result is {response["result"]}, expected OK'
...
def check_get_bank_account_result(response, result):
    assert response['result'] == result, f'get-bank-account response is not ok. Result is {response["result"]}, expected {result}'
...
def delete_bank_account(resp):
    ...
    response = Circle().delete_bank_account(resp['token'], bankaccountid_to_delete)['response']
...
def check_delete_bank_account_response(response):
    assert response['result'] == 'OK', f'delete_bank_account response is not ok. Result is {response["result"]}, expected OK'
    assert response['data']['deleted'] is True, f'delete_bank_account response is not ok. Deleted is {response["data"]["deleted"]} expected "true"'

@then("get-bank-accounts-all haven't to return deleted bank account")
def check_delete_bank_account(resp, response):
    r = Circle().get_bank_account_all(resp['token'])['response']
    assert len(r['data']) == resp['accounts_count'] - 1, f'get-bank-accounts-all response is not ok. {len(response["data"]["data"])} accounts at response, expected {resp["accounts_count"] - 1}'
...
def check_delete_deleted_bank_account(resp):
    r = Circle().get_bank_account_all(resp['token'])['response']
    assert len(r['data']) == resp['accounts_count'], f'get-bank-accounts-all response is not ok. {len(r["data"])} accounts at response, expected {resp["accounts_count"]}'
...
def delete_bank_account_2(auth, resp):
    ...
    response = Circle().delete_bank_account(token, resp['bankaccountid_to_delete'])['response']
    return response

@when('user send request to delete bank account without token', target_fixture='response')
def delete_bank_account_3(resp):
    response = Circle().delete_bank_account('', resp['bankaccountid_to_delete'], specific_case=True)
    return response
