...
@scenario('../features/circle.feature', 'Add bank accounts')
@then('response must contains created bank account info')
def check_add_bank_account_response_data(bank_country, billing_country, account_number, routing_number, iban,
                                         add_bank_account_response: dict):
    response = add_bank_account_response.get('data')
    ...
    assert isinstance(response_data['clientId'], str), f'add_bank_account response is not ok. ClientId is {response_data["clientId"]}, expected str'
    ...
    assert response_data['billingDetailsCountry'] == billing_country, f'add_bank_account response is not ok. BillingDetailsCountry is {response_data["billingDetailsCountry"]}, expected "{billing_country}"'
    ...
    assert response_data['bankAddressCountry'] == bank_country, f'add_bank_account response is not ok. BankAddressCountry is {response_data["bankAddressCountry"]}, expected "{bank_country}"'
    ...
    if iban == 'null':
        ...
        assert response_data['iban'] == iban, f'add_bank_account response is not ok. Iban is {response_data["iban"]}, expected "{iban}"'
    if account_number == 'null':
        ...
        assert response_data['accountNumber'] == account_number, f'add_bank_account response is not ok. AccountNumber is {response_data["accountNumber"]}, expected {account_number}'
    if routing_number == 'null':
        ...
        assert response_data['routingNumber'] == routing_number, f'add_bank_account response is not ok. RoutingNumber is {response_data["routingNumber"]}, expected {routing_number}'

@given(parsers.parse('bank_country is {bank_country}'), target_fixture="bank_country")
def get_bank_country(bank_country):
    return bank_country

@given(parsers.parse('billing_country is {billing_country}'), target_fixture="billing_country")
def get_billing_country(billing_country):
    return billing_country

@given(parsers.parse('account_number is {account_number}'), target_fixture="account_number")
def get_account_number(account_number):
    return account_number

@given(parsers.parse('iban is {iban}'), target_fixture="iban")
def get_iban(iban):
    return iban

@given(parsers.parse('routing_number is {routing_number}'), target_fixture="routing_number")
def get_routing_number(routing_number):
    return routing_number

@given(parsers.parse('guid is {guid}'), target_fixture="guid")
def get_guid(guid):
    return guid
...
def check_response_status_code(status_code, add_bank_account_response: dict):
    response_status_code = add_bank_account_response.get('status')
